package org.tsob.MobDrop3.DataBase.MySQL;
...
import org.tsob.MobDrop3.DataBase.Callback;
...
//      DataBase.Print("can not get DataBase [ " + db + " ]");
      ...
      //Handle errors for JDBC
      MobDrop3.plugin.getLogger().log(Level.WARNING, "SQLException: ");
      se.printStackTrace();
    }catch(Exception e){
      e.printStackTrace();
    }finally{
...
//      }
  ...
  public void executeUpdateAsync(String command, Callback<Boolean> callback) {
    Bukkit.getScheduler().runTaskAsynchronously(MobDrop3.plugin, () -> {
      try {
  ...
  protected void print(String color,String message) {
    if(MobDrop3.plugin.getConfig().getBoolean("SQL.Info")) {
